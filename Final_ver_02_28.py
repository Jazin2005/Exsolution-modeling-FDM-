    ...
    def __call__ (self):
        ...
        for i in range (0, self.itr+1):
            if i ==0:
                ...
                ims_1D = []
                im_1, = ax1.plot(np.linspace(1, self.nx, self.ny),c[self.x,:], marker="o", color='r')
                fig1.suptitle('Concentration vs X Direction', fontsize=20)
                ax1.set_ylabel('Concentration', fontsize=15)
                ax1.set_xlabel('X direction', fontsize=15)
                ...
                im_2, = ax2.plot(cc, self.gibbs_func (cc, W_or_ab, W_ab_or), 
                                color='g', linewidth=4, label="Gibss free energy")

                c_int=0.55
                gibbs_en_al = self.gibbs_func (c_int, self.W_or_ab, self.W_ab_or)
                ax2.plot(c_int, gibbs_en_al, color='b', marker="o", markersize=5)
                ims_gibbs.append([im_2])
                ...
                if i%self.skip==0:
                    ...
                    cc = np.linspace(0.01, 0.99, 100)
                    im_2, = ax2.plot(cc, self.gibbs_func (cc, W_or_ab, W_ab_or), 
                                color='g', linewidth=2, label="Gibss free energy", alpha =0.4)
                    
                    c_n=c_new[5,5]
                    gibbs_en_al = self.gibbs_func (c_n, self.W_or_ab, self.W_ab_or)
                    im_2, = ax2.plot(c_n, gibbs_en_al, color='b', marker="o", markersize=12)

                    gibbs_en_al = self.gibbs_func (1-c_n, self.W_or_ab, self.W_ab_or)
                    im_2, = ax2.plot(1-c_n, gibbs_en_al, color='r', marker="o", markersize=12)
        ...
        Movie = animation.ArtistAnimation(fig2, ims_gibbs, interval=200, blit=True)
        Movie.save("C:/Users/Javad/Desktop/res.mp4")

        #Movie_1 = animation.ArtistAnimation(fig1, ims_1D, interval=200, blit=True)
        #Movie_1.save("C:/Users/Javad/Desktop/r.mp4")
...
T = 650 # temperature [K]
P = 5 # Pressure [bar]
itr = 1000# total number of iteration
...
x = 10    # 23, 17 # The grid number in x direction for visualization. 
          # This variable should be less or equal than nx
y = 10    # The grid number in x direction for visualization.
